...
interface WikiSummaryInputs {
  subject: string
}

export async function main(inputs: WikiSummaryInputs) {
    ...
    messages: [
      {
        role: 'system',
        content:
          `Guess what the Wikipedia articles title would be, for the given subject matter: ${subject}. ONLY respond with a few words and nothing else, NO explanation.`,
      },
    ],
  })
...
if (import.meta.main) {
  await main({
    subject: Deno.args[0] || options.inputs.find((input) =>
      input.name === 'subject'
